...
func (f *GeneratedGoFile) Imports() []string {
	...
	return maputils.MapKeys(imports)
}
...
func generateStruct(ctx *GoProcessingContext, evt spec.Spec) error {
	templateCode := `
const {{ .StructName }}Name string = "{{ .Name }}"
...
func generateCommand(ctx *GoProcessingContext, cmd spec.Spec) error {
	templateCode := `
const {{ .StructName }}Name command.Name = "{{ .Name }}"
...
func (c {{ .StructName }}) Name() command.Name {
	return {{ .StructName }}Name
}
`
...
func generateQuery(ctx *GoProcessingContext, query spec.Spec) error {
	templateCode := `
const {{ .StructName }}Name query.Name = "{{ .Name }}"
...
func (c {{ .StructName }}) Name() query.Name {
	return {{ .StructName }}Name
}
`
...
func generateEvent(ctx *GoProcessingContext, evt spec.Spec) error {
	templateCode := `
const {{ .StructName }}Name event.Name = "{{ .Name }}"
...
func (c {{ .StructName }}) Name() event.Name {
	return {{ .StructName }}Name
}
`
...
func {{ .EndpointFuncName }}(r chi.Router, bus {{ if eq .Method "POST" }}command.Bus{{ else }}event.Bus{{ end }}) {
	r.Get("{{ .Path }}", func(w http.ResponseWriter, r *http.Request) {
		handleError := func(w http.ResponseWriter, r *http.Request, err error) {
			...
			c := conv[derr.Name()]
			w.WriteHeader(c)
			render.JSON(w, r, NewErrorResponse(derr.Name(), derr.Error(), derr.Data()))
		}
